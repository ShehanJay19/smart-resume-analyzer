...
export const SupervisorAgent = () => {
  ...
  const handleRun = async () => {
    if (!jobDescription.trim()) {
      setResult({
        detail: "Paste a job description to run the supervisor agent.",
      });
      return;
    }
    ...
    try {
      ...
      setResult({ detail: "Unable to run the supervisor agent. Upload a resume." });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
      <div className="surface-card p-6">
        ...
        <h2 className="mt-3 font-display text-3xl text-ink-900">
          Orchestrate a multi-agent resume review.
        </h2>
        ...
        <div className="mt-4 text-sm text-ink-700">
          {result?.detail ? (
            ...
            <p className="text-ink-500">
              Run the supervisor agent to see the multi-agent output.
            </p>
          )}
        </div>
      </div>
